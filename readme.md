...
Each city is then is assigned a number of cells from the suitability raster of each potential biomass resource from Olive and Vineyards according to the Voronoi polygons in figure above. The suitability of each cell is defined as mentioned in the work of Kutchartt et al. [] as the following:
...
-   distance from nearest road: a cell will be more suitable than another, given the same NDVI values and other conditions, if closer to the road, thus will have a higher suitability score.

Again it is to note that the suitability score is relative and unitless, and is used to infer which cell is more suitable than another. Each city is thus given the sum of suitability scores of the cells that fall inside the Voronoi polygon, also inversely weighted linearly by distance, as we want to account for transportation cost from the cell to the city where the pyrolysis plant is to be built.

One limitation that might be argued about is that assigning cells to cities according to Voronoi polygons will give greater gain to isolated cities, because of the larger Voronoi polygon, as opposed to areas with several cities near each other. This is true, but is actually a further positive aspect as isolated cities will undoubtedly not have to share residual biomass with other nearby cities. Another positive aspect is that clustered cities will collect residual biomass from a smaller area (smaller Voronoi polygon), but if one or more nearby cities are discarted, the residual biomass from discarted neighbour cities will go to the "surviving" city, further adding gain and thus supporting its selection.
